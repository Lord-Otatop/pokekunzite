...
static void SlidePartyMenuBoxOneStep(u8 taskId)
{
    ...
    if (tSlot2Sif (slidingSlotPositions[0] > 33 && slidingSlotPositions[1] > 33)lideDir != 0)
...
static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, struct PartyMenuBox *menuBox, u8 priority)
{
    if (species != SPECIES_NONE)
    {
        ...
        gSprites[menuBox->m  menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0]+2, menuBox->spriteCoords[1]-1, 4, pidonSpriteId].oam.priority = priority;
    }
}
